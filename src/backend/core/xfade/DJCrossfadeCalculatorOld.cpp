...
const std::string DJCrossfadeCalculatorOld::m_bpmInfoFileExt = ".bpm";

DJCrossfadeCalculatorOld::DJCrossfadeCalculatorOld(const AudioFile & audioFile1, const AudioFile & audioFile2)
...
DJCrossfadeCalculatorOld::~DJCrossfadeCalculatorOld() {
	// TODO Auto-generated destructor stub
}

bool DJCrossfadeCalculatorOld::ReadBeatgrid(std::ifstream & ifs, double & instantaneousFadeTempo, std::vector<double> & fadeInfo)
{
	bool rv = false;
...
bool DJCrossfadeCalculatorOld::ReadFadeInfo(std::ifstream & ifs, bool fadeOut, double & fadeTime, double & instantaneousFadeTempo, std::vector<double> & fadeInfo)
{
	bool rv = false;
...
bool DJCrossfadeCalculatorOld::VerifyRubberBandConstraintsForSingleGrid(const std::vector<double> & fadeInfo, size_t (*bgOffsetFunc)(const DJCrossfadeCalculatorOld &)) const
{
	...
	for (it = fadeInfo.begin() + k; it != fadeInfo.end(); ++it)
	{
		...
		double stretchFactor2 = normalBPM / stretchedBPM2;
...
bool DJCrossfadeCalculatorOld::VerifyRubberBandConstraints() const
{
	return VerifyRubberBandConstraintsForSingleGrid(m_FadeOutInfo,
			[](const DJCrossfadeCalculatorOld & calc)
...
double DJCrossfadeCalculatorOld::GetBPMAtStartOfCrossfade() const
{
	return m_FadeOutInfo.empty() ? m_InstantaneousFadeOutTempo
	                             : *m_FadeOutInfo.begin();
}

double DJCrossfadeCalculatorOld::GetBPMAtEndOfCrossfade() const
{
	return m_FadeInInfo.empty() ? m_InstantaneousFadeInTempo
	                            : *m_FadeInInfo.rbegin();
}

size_t DJCrossfadeCalculatorOld::GetFadeOutBeatgridOffset() const
{
	return 0; // This is zero, no matter what!
	// (I leave this defined to make the code look balanced and elegant.)
}

size_t DJCrossfadeCalculatorOld::GetFadeInBeatgridOffset() const
{
	size_t offset = 0;
...
size_t DJCrossfadeCalculatorOld::GetMinBeatgridSize() const
{
	return std::min(m_FadeInInfo.size(), m_FadeOutInfo.size());
}

size_t DJCrossfadeCalculatorOld::GetMaxBeatgridSize() const
{
	return std::max(m_FadeInInfo.size(), m_FadeOutInfo.size());
}

double DJCrossfadeCalculatorOld::GetTimeAtFadeOutBeat(size_t beatPos) const
{
	double delta = 0.0;
	size_t count = 0;
...
double DJCrossfadeCalculatorOld::GetTimeAtEndOfFadeOut() const
{
	return GetTimeAtFadeOutBeat(m_FadeOutInfo.size());
}

double DJCrossfadeCalculatorOld::GetTimeAtFadeInBeat(size_t beatPos) const
{
	double delta = 0.0;
	size_t count = 0;
...
double DJCrossfadeCalculatorOld::GetTimeAtEndOfFadeIn() const
{
	return GetTimeAtFadeInBeat(m_FadeInInfo.size());
}

double DJCrossfadeCalculatorOld::InterpolateBPMAtCrossfade(double percent) const
{
...
double DJCrossfadeCalculatorOld::ComputePercentageChangeForFadeOutTrack(double percent, double time, double timeChange, double & newTime, bool & done)
{
...
double DJCrossfadeCalculatorOld::ComputePercentageChangeForFadeInTrack(double percent, double time, double timeChange, double & newTime, bool & done)
{
	newTime = time + timeChange;
...
double DJCrossfadeCalculatorOld::ComputeOriginalTimeChangeForFadeOutTrack(double percent, double percentChange)
{
...
double DJCrossfadeCalculatorOld::ComputeOriginalTimeChangeForFadeInTrack(double percent, double percentChange)
{
	double timeChange = 0.0;
	double dStartIndex = percent * GetMaxBeatgridSize();
	size_t startIndex = (size_t) dStartIndex;
...
double DJCrossfadeCalculatorOld::ComputeVolumeForFadeInTrack(double percent) const
{
...
double DJCrossfadeCalculatorOld::ComputeStretchFactorForFadeOutTrack(double percent) const
{
	double factor = 1.0;
	size_t index = (size_t) (percent * GetMaxBeatgridSize()); // NB:  This is the global index, not the beatgrid-specific index
	size_t start = GetFadeOutBeatgridOffset();
	if (index >= start && index < start + m_FadeOutInfo.size())
	{
		factor = m_FadeOutInfo.at(index) / InterpolateBPMAtCrossfade(percent);
	}
	return factor;
}

double DJCrossfadeCalculatorOld::ComputeStretchFactorForFadeInTrack(double percent) const
{
	double factor = 1.0;
	size_t index = (size_t) (percent * GetMaxBeatgridSize()); // NB:  This is the global index, not the beatgrid-specific index
	size_t start = GetFadeInBeatgridOffset();
	...
	{
		index -= start;
		factor = m_FadeInInfo.at(index) / InterpolateBPMAtCrossfade(percent);
	}
	return factor;
}

double DJCrossfadeCalculatorOld::GetFadeOutPercentage() const
{
	size_t maxBGSize = GetMaxBeatgridSize();
	return maxBGSize == 0 ? 0.0 : GetFadeOutBeatgridOffset() / ((double) maxBGSize);
}

double DJCrossfadeCalculatorOld::GetFadeInPercentage() const
{
	size_t maxBGSize = GetMaxBeatgridSize();
	return maxBGSize == 0 ? 0.0 : GetFadeInBeatgridOffset() / ((double) maxBGSize);
}

bool DJCrossfadeCalculatorOld::ReadBPMFiles()
{
	bool rv = false;
...
double DJCrossfadeCalculatorOld::GetTimeAtStartOfFadeOut() const
{
	return m_FadeOutTime;
}

double DJCrossfadeCalculatorOld::GetTimeAtStartOfFadeIn() const
{
	return m_FadeInTime;
}

bool DJCrossfadeCalculatorOld::CheckCrossfadeCondition()
{
	bool rv = false;
